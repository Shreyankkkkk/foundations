...
a=int(input(f"{CYAN}Enter Password : {ENDC}"))
conn = sql.connect(
    host='localhost',
    user='root',
    passwd=str(a),
...
print(f'\n{BOLD}{CYAN}--------------------Monte Carlo Simulation--------------------{ENDC}\n')
...
if mode == '1':
    ...
    while True:
        print("\nRange is 0-10, realistically it doesnt increase above that")
...
while True:
    print
    account_type = int(input("Run for (1) Live Account / (2) Prop firm Trading : "))
    if account_type in [1,2]:
        if account_type == 1:
            target_goal = float(input("Enter Target Goal :")) 
        elif account_type == 2:
            prop_lost = 0
            print("Prop Firm Trading selected. Enter Rules")
            not_accepted_max_dd = float(input("Maximum Drawdown allowed :")) #if account reaches this drawdown, it is not accepted, simulation stops
        break
...
def run_single_simulation(account_size, number_of_trades):
    global total_profit, total_loss, count3, prop_lost, account_type
    wX = []
    vY = []
...
if account_type == 1:
    ...
    print(f"Number of Accounts Meeting Target Goal (${target_goal}): {accounts_meeting_goal}")
    print(f"Percentage of Accounts Meeting Target Goal (${target_goal}): {percentage_met_goal:.2f}%")
    ...
    print(f'Your target goal of ${target_goal} is approximately at the {goal_percentile:.2f}th percentile of all accounts.')
...
if account_type == 2:
    ...
    print("Number of Accounts Failed (>= -8%) :", prop_lost)
    print("Number of Accounts Not Failed :", passed_accounts)
    print()
    ...
    print("Number of Accounts Passed (>= 0%) :", passed_0)
    print("Number of Accounts Passed (>= 6%) :", passed_6)
    print("Number of Accounts Passed (>= 12%) :", passed_12)
    print("Number of Accounts Passed (>= 22%) :", passed_22)
    print()

    # Now calculate the percentages based on the total number of accounts
    print("Accounts >= -8% :", (prop_lost / total_number_of_accounts) * 100, "%")
    print("Accounts that weren't lost :", (passed_accounts / total_number_of_accounts) * 100, "%")
    print()

    # Now calculate percentages within the passed accounts
    if passed_accounts > 0:
        print("In all the Passed Accounts :")
        print(f"Failed Phase 1 : {(passed_0 / passed_accounts) * 100} %")
        print(f"Failed Phase 2, Passed Phase 1 : {(passed_6 / passed_accounts) * 100} %")
        print(f"Passed Phase 1 and Phase 2 : {(passed_12 / passed_accounts) * 100} %")
        print(f"Passed Both Phases and Received Payout : {(passed_22 / passed_accounts) * 100} %")
    print()
...
(3) - 75,000
(9) - 5,000,000
(10) - 500,000 = 100 trades, 5 runs, 1000 people
'''
