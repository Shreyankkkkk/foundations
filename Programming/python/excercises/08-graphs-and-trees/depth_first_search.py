# Project: Depth First Search Algorithm
'''
...
def dfs(adjacency_matrix: list, nodes_start: int):
    ...
    while stack:
        node = stack.pop()
        result.add(node)
        for children in range(number_of_nodes):
            if adjacency_matrix[node][children] == 1 and children not in result:
