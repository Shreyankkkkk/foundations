...
def selection_sort(array):
    for index in range(len(array)):
        current = index
        minimum = index
        for in_index in range(index + 1, len(array)):
            if array[in_index] < array[minimum]:
                minimum = in_index

        current_value = array[current]
        minimum_value = array[minimum]
        if array[current] != minimum_value:
            array[current] = minimum_value
            array[minimum] = current_value
    return array
